  ...
  <div id="game">
    <el-row>
      <el-card shadow="never">
        <el-page-header @back="goBack" content="绿洲连连看">
        </el-page-header>
      </el-card>
    </el-row>
    ...
    <el-row>
      <el-card shadow="never" style="width: 100%">
        <div :class="currentTheme.name">
          <div class="game-config" align="center">
            <h2>倒计时： {{ clock }} </h2>
            <el-button type="button" @click="init">重新开始</el-button>
          </div>

          <table class="game" @click="handleClick">
            <tr :key="row" v-for="(cols, row) in cellData">
              <cell :key="col" v-for="(cell, col) in cols"
...
export default {
  ...
  watch: {
    ...
    cnt_line(nval, oval) {
      if (nval >= 18) {
        ...
        var _this = this;
        _this.axios.post('/api/tflz/game/record/' +
            this.$store.getters.getUserInfo.id + '/' +
            this.clock)
            .then(function () {
              _this.$alert('成功！', '天府绿洲 TFLZ Alpha 1.0', {
                confirmButtonText: '确定'
              });
              _this.$router.push('/Linkup');
            })
  ...
  mounted() {
    this.$alert('手机用户请横屏以获得更好体验！', '天府绿洲 TFLZ Alpha 1.0', {
      confirmButtonText: '确定'
    });
  ...
  methods: {
    ...
    init() {
      ...
      var timeClock = function () {
        if (_this.clock <= 0) {
          this.$alert('时间到，挑战失败！', '天府绿洲 TFLZ Alpha 1.0', {
            confirmButtonText: '确定'
          });
          window.clearInterval(_this.internalId);
          _this.internalId = -1;
          return;
        }
        _this.clock = _this.clock - 1;
      }
      this.internalId = window.setInterval(timeClock, 1000);
